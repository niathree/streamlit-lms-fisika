# lms_dinamika_rotasi_final_fix_v4.py
...
FILES = {
    ...
    "pre_test_p1": "pre_test_pertemuan_1.json",
    "pre_test_p2": "pre_test_pertemuan_2.json",
    ...
    if menu == "Daftar Hadir":
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            if submitted:
                ...
                with open(NOTIFIKASI_FILE, "r") as f:
                    notif = json.load(f)
                notif["Video Apersepsi"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(NOTIFIKASI_FILE, "w") as f:
                    json.dump(notif, f)
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            st.header("🧠 Kelola Soal Pre-test")
            ...
            with tab1:
                ...
                with st.form("form_edit_pre_p1"):
                    ...
                    submitted = st.form_submit_button("💾 Simpan Soal Pre-test Pertemuan 1")
                if submitted:
                    data_baru = {
                        "judul": judul_baru,
                        "deskripsi": desc_baru,
                ...
                # Tampilkan pratinjau
                st.divider()
                st.subheader("👁️‍🗨️ Pratinjau Pre-test Pertemuan 1 untuk Siswa")
                st.write(f"**{data_p1.get('judul', 'Pre-test')}**")
                st.info(data_p1.get("deskripsi", ""))
                for i, soal in enumerate(data_p1.get("soal_list", []), 1):
            ...
            with tab2:
                ...
                with st.form("form_edit_pre_p2"):
                    ...
                    submitted = st.form_submit_button("💾 Simpan Soal Pre-test Pertemuan 2")
                if submitted:
                    data_baru = {
                        "judul": judul_baru,
                        "deskripsi": desc_baru,
                ...
                # Tampilkan pratinjau
                st.divider()
                st.subheader("👁️‍🗨️ Pratinjau Pre-test Pertemuan 2 untuk Siswa")
                st.write(f"**{data_p2.get('judul', 'Pre-test')}**")
                st.info(data_p2.get("deskripsi", ""))
                for i, soal in enumerate(data_p2.get("soal_list", []), 1):
                    st.markdown(f"#### {i}. {soal['teks']}")
            ...
            # KOREKSI: Ganti 'pre-test_data' menjadi 'data'
            if not data:
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            if submitted:
                ...
                with open(NOTIFIKASI_FILE, "r") as f:
                    notif = json.load(f)
                notif["Deskripsi Materi"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(NOTIFIKASI_FILE, "w") as f:
                    json.dump(notif, f)
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            with tab1:
                ...
                if st.button("➕ Tambahkan Video/Gambar Pertemuan 1"):
                    ...
                    with open(NOTIFIKASI_FILE, "r") as f:
                        notif = json.load(f)
                    notif["Media Pembelajaran"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    with open(NOTIFIKASI_FILE, "w") as f:
                        json.dump(notif, f)
                    st.rerun()
                # Tombol Simpan
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            for i, sim in enumerate(simulasi_data.get("simulasi_list", []), 1):
                ...
                # PhET tidak bisa diembed langsung, jadi gunakan link
            ...
            simulasi_list = simulasi_data.get("simulasi_list", [])
            if simulasi_list:
                simulasi = simulasi_list[0] # Ambil simulasi pertama (Balancing Act)
                st.markdown(f"#### {simulasi['judul']}")
                st.caption(f"Sumber: {simulasi['sumber']}")
                # KOREKSI: Gunakan URL yang benar untuk embed
                st.link_button("🔗 Buka Simulasi PhET: Balancing Act", simulasi["url"])
            else:
                st.warning("📁 Simulasi PhET Balancing Act belum diatur oleh guru.")
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            if submitted:
                ...
                with open(NOTIFIKASI_FILE, "r") as f:
                    notif = json.load(f)
                notif["LKPD"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(NOTIFIKASI_FILE, "w") as f:
                    json.dump(notif, f)
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            # KOREKSI: Ganti 'refleksi_siswa' menjadi 'refleksi'
            ...
            if submitted:
                ...
                simpan_data("refleksi", data_baru) # KOREKSI: Ganti 'refleksi_siswa' menjadi 'refleksi'
                # Update notifikasi
                with open(NOTIFIKASI_FILE, "r") as f:
                    notif = json.load(f)
                notif["Refleksi Siswa"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(NOTIFIKASI_FILE, "w") as f:
                    json.dump(notif, f)
        ...
        if st.session_state.role in ["guru", "admin"]:
            # --- DASBOR GURU: POST-TEST ---
            st.header("📝 Edit Soal Post-test")
            post_test_data = muat_data("post_test")
            if not post_test_data:
                st.error("Soal post-test belum diatur.")
                st.stop()
            with st.form("form_edit_post_test"):
                judul_baru = st.text_input("Judul Post-test", value=post_test_data.get("judul", ""))
                desc_baru = st.text_area("Deskripsi Post-test", value=post_test_data.get("deskripsi", ""), height=150)
                st.subheader("📄 Daftar Soal Post-test")
            ...
            if submitted:
                ...
                with open(NOTIFIKASI_FILE, "r") as f:
                    notif = json.load(f)
                notif["Post-test"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(NOTIFIKASI_FILE, "w") as f:
                    json.dump(notif, f)
        ...
        if st.session_state.role in ["guru", "admin"]:
            ...
            if kirim and pesan.strip():
                df = pd.read_csv(FILES["forum_diskusi"])
